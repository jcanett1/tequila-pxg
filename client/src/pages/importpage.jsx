...
export function Importpage() {
    ...
    const fetchItems = async (page=1) => {
      ...
      try {
        const response = await axios.get('http://localhost:8000/tasks/data/import/', {
          params: {
        ...
        setSumImport(response.data.sumImport); 
      } catch (error) {
    ...
    const handleDownloadExcel = async () => {
      ...
      try {
        ...
        const url = `http://localhost:8000/tasks/import/?start_date=${startDate}&end_date=${endDate}&inventory_id=${inventoryid}&invoice_=${invoice}`;
        ...
        link.href = urlObject;
        link.setAttribute('download', 'import_data.xlsx');
    ...
    return (
      <div>
        <p className='title' style={{marginTop:50, marginBottom:5, marginLeft:20}}>Import Data</p>
        ...
        <ul style={{marginLeft:20, marginRight: 20}}>
          <table className='table-container'>
            <thead>
              <tr>
                ...
                <th scope='col'style={{width: '250px', textAlign:'left'}}> Unit Cost</th>
                <th scope='col'style={{width: '250px', textAlign:'left'}}> Total</th>
              </tr>
            </thead>
            <tbody>
              {items.map(item => (
          ...
          <div><p>Total Import: {sumImport}</p></div>
        </ul>
